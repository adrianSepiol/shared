 ...
 */
export function getRepeatVariableValues(repeatVariable: RepeatVariable, variableValues: VariableStateMap): string[] {
...
/**
 * Returns how many items fit in a single row for a repeat variable.
 */
...
export function buildRepeatMeta(
  ...
  const expandedItemLayouts = itemLayouts.map((itemLayout) => {
    ...
    const values = getRepeatVariableValues(itemRepeatVariable, variableValues);
